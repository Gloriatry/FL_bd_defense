...
import random
import copy
def test_img(net_g, datasethelper, args, test_backdoor=False):
    args.watermark = None
    args.apple = None
    net_g.eval()
    # testing
    ...
    back_correct = 0
    back_num = 0
    if args.attack == 'no_attack' and args.heter_method == 'mnist7':
        data_loader_other = DataLoader(datasethelper.dataset_test,
                            batch_size=args.bs,
                            sampler=torch.utils.data.sampler.SubsetRandomSampler(datasethelper.test_other_idx))
        data_loader_7 = DataLoader(datasethelper.dataset_test,
                            batch_size=args.bs,
                            sampler=torch.utils.data.sampler.SubsetRandomSampler(datasethelper.test_7_idx))
        data_loader_bar7 = DataLoader(datasethelper.dataset_test,
                            batch_size=args.bs,
                            sampler=torch.utils.data.sampler.SubsetRandomSampler(datasethelper.test_bar7_idx))
        _, acc_other = test_acc(copy.deepcopy(net_g), data_loader_other, len(datasethelper.test_other_idx), args)
        _, acc_7 = test_acc(copy.deepcopy(net_g), data_loader_7, len(datasethelper.test_7_idx), args)
        _, acc_bar7 = test_acc(copy.deepcopy(net_g), data_loader_bar7, len(datasethelper.test_bar7_idx), args)
        
        return acc_other, acc_7, acc_bar7

    if args.attack == 'semantic':
        data_loader = DataLoader(datasethelper.dataset_test, batch_size=args.bs)
        l = len(data_loader)
        ...
        if test_backdoor:
            poisoned_test_loader = DataLoader(datasethelper.dataset_test,batch_size=args.bs,
                                              sampler=torch.utils.data.sampler.SubsetRandomSampler(range(1000)))
            for batch_id, (images, labels) in enumerate(poisoned_test_loader):
                for pos in range(len(images)):
                    images[pos] = datasethelper.dataset_train[random.choice(args.poison_images_test)][0]
                    labels[pos] = args.poison_label_swap
                if args.gpu != -1:
                    images, labels = images.to(args.device), labels.to(args.device)
                log_probs = net_g(images)
                # get the index of the max log-probability
                y_pred = log_probs.data.max(1, keepdim=True)[1]
                back_correct += y_pred.eq(labels.data.view_as(y_pred)).long().cpu().sum()
            back_num = len(poisoned_test_loader.dataset)
        test_loss /= len(data_loader.dataset)
        accuracy = 100.00 * correct / len(data_loader.dataset)             
    elif args.attack == 'edges':
        vanilla_test_loader = datasethelper.vanilla_test_loader
        targetted_task_test_loader = datasethelper.targetted_task_test_loader
        for idx, (data, target) in enumerate(vanilla_test_loader):
            if args.gpu != -1:
                data, target = data.to(args.device), target.to(args.device)
            log_probs = net_g(data)
            # sum up batch loss
            ...
            correct += y_pred.eq(target.data.view_as(y_pred)).long().cpu().sum()
        if test_backdoor:
            for idx, (data, target) in enumerate(targetted_task_test_loader):
                if args.gpu != -1:
                    data, target = data.to(args.device), target.to(args.device)
                log_probs = net_g(data)
                # get the index of the max log-probability
            ...
            back_num = len(targetted_task_test_loader.dataset)
        test_loss /= len(vanilla_test_loader.dataset)
        accuracy = 100.00 * correct / len(vanilla_test_loader.dataset)
    else:
        data_loader = DataLoader(datasethelper.dataset_test, batch_size=args.bs)
        l = len(data_loader)
        for idx, (data, target) in enumerate(data_loader):
...
def test_acc(net, data_loader, data_num, args):
    net.eval()
    correct = 0
    test_loss = 0
    for idx, (data, target) in enumerate(data_loader):
